...
test('compose api cli main persists ids after discovery and creation', async () => {
  ...
  await main(['--env-file', envFilePath, '--prepared'], {
    ...
    fetchFn: async (url, options = {}) => {
      ...
      if (url.includes('/project.all')) return { ok: true, async json() { return []; } };
      if (url.includes('/project.one') && options.method !== 'POST') return { ok: true, async json() { return { environments: [] }; } };
  ...
  assert.deepEqual(operationLog, [
    ['fetch', 'https://dokploy.example.com/api/project.all', 'GET'],
    ['fetch', 'https://dokploy.example.com/api/project.create', 'POST'],
    ['fetch', 'https://dokploy.example.com/api/project.one?projectId=project-1', 'GET'],
    ['fetch', 'https://dokploy.example.com/api/environment.create', 'POST'],
    ['fetch', 'https://dokploy.example.com/api/project.one?projectId=project-1', 'GET'],
    ['fetch', 'https://dokploy.example.com/api/compose.create', 'POST'],
    ['render', tempDir],
    ['fetch', 'https://dokploy.example.com/api/compose.update', 'POST'],
    ['fetch', 'https://dokploy.example.com/api/compose.deploy', 'POST'],
  ]);
});
...
test('compose api cli main persists discovered ids without creating resources', async () => {
  ...
  await main(['--env-file', envFilePath, '--prepared'], {
    ...
    fetchFn: async (url, options = {}) => {
      operationLog.push(['fetch', url, options.method ?? 'GET']);
      if (url.includes('/project.all')) return { ok: true, async json() { return [{ name: 'zrok', projectId: 'project-existing' }]; } };
      if (url.includes('/project.one')) return { ok: true, async json() { return { environments: [{ name: 'production', environmentId: 'env-existing', compose: [{ name: 'zrok-instance', appName: 'zrok-instance', composeId: 'compose-existing' }] }] }; } };
      return { ok: true, async json() { return {}; } };
    },
    renderComposeBundle: async (instanceDirectoryPath) => {
      operationLog.push(['render', instanceDirectoryPath]);
  ...
  assert.deepEqual(operationLog, [
    ['fetch', 'https://dokploy.example.com/api/project.all', 'GET'],
    ['fetch', 'https://dokploy.example.com/api/project.one?projectId=project-existing', 'GET'],
    ['fetch', 'https://dokploy.example.com/api/project.one?projectId=project-existing', 'GET'],
    ['render', tempDir],
    ['fetch', 'https://dokploy.example.com/api/compose.update', 'POST'],
    ['fetch', 'https://dokploy.example.com/api/compose.deploy', 'POST'],
